	...
	<c:choose>
		...
		<c:when test="${step eq '3'}">
			<li>
				...
				<c:set var="str" value="지금은 c:set 연습 중"/>
				<c:out value="${str}"/>
			</li>
		</c:when>
		...
		<c:when test="${step eq '7'}">
			<li>
				<h3>c:param Tag : 포함하는 import, url 태그의 URL 매개 변수를 추가</h3>
		...
		<c:when test="${step eq '12'}">
			<li>
				<h3>fn:endsWith() : 주어진 문자열이 특정 문자열 값으로 끝나는 지 확인하는 데 사용</h3>
		...
		<c:when test="${step eq '13'}">
			<li>
				<h3>fn:endsWith() : 주어진 문자열이 특정 문자로 구분해서 배열로 분할</h3>
		...
		<c:when test="${step eq '19'}">
			<li>
				<h3>fn:trim() : 문자열의 양쪽 끝에서 공백을 제거(문자열 사이 공백은 안 사라짐)</h3>
		...
		<c:when test="${step eq '20'}">
			<li>
				<h3>fmt:formatDate : 제공된 패턴 및 스타일을 사용해 시간 및 날짜를 형식화함</h3>
